...
declare module kefir {
  declare class Observable<T, E> {
    ...
    combine<U, F, R>(
      otherObs: Observable<U, F>,
      combinator?: (a: T, b: U) => R
    ): Stream<R, E | F>;
    zip<U, F, R>(
      obs: Observable<U, F> | U[],
      combinator?: (a: T, b: U) => R
    ): Stream<[T, U] | R, E | F>;
    ...
    flatMapConcurLimit<U, F>(
      fn: ?(value: T) => Observable<U, F>,
      limit: number
    ): Stream<U, E | F>;
  ...
  declare function withInterval<T, E>(
    period: number,
    handler: (e: Emitter<T, E>) => any
  ): Stream<T, E>;
  declare function fromCallback<T>(
    fn: (callback: (value: T) => void) => any
  ): Stream<T, any>;
  declare function fromNodeCallback<T, E>(
    fn: (callback: (err: E, value: T) => void) => any
  ): Stream<T, E>;
  declare function fromEvents<T>(
    target: EventTarget | events$EventEmitter,
    eventName: string,
    transform?: (...args: any[]) => T
  ): Stream<T, any>;
  declare function stream<T, E>(
    subscribe: (e: Emitter<T, E>) => ?() => any
  ): Stream<T, E>;
  ...
  declare function combine<T, E, U, F, R>(
    obss: Observable<T, E>[],
    passiveObs?: Observable<U, F>[],
    combinator?: (...values: (T | U)[]) => R
  ): Stream<R, E | F>;

  declare var zip: (<A, EA, B, EB>(
    sources: [Observable<A, EA> | A[], Observable<B, EB> | B[]]
  ) => Stream<[A, B], EA | EB>) &
    (<A, EA, B, EB, C, EC>(
      sources: [
        Observable<A, EA> | A[],
    ...
    (<T, E, R>(
      sources: (Observable<T, E> | T[])[],
      combinator: (...values: T[]) => R
    ) => Stream<R, E>);

  declare var merge: (<A, EA, B, EB>(
    obss: [Observable<A, EA>, Observable<B, EB>]
  ) => Stream<A | B, EA | EB>) &
    (<A, EA, B, EB, C, EC>(
      obss: [Observable<A, EA>, Observable<B, EB>, Observable<C, EC>]
    ) => Stream<A | B | C, EA | EB | EC>) &
    (<A, EA, B, EB, C, EC, D, ED>(
      obss: [
        Observable<A, EA>,
