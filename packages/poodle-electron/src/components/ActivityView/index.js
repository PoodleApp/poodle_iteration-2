...
function ActivityCard(props: { nestLevel: ?number, children?: any }) {
  return <div style={styles.activityCard}>
...
function NoteView(props: ActivityViewProps) {
  ...
  return <ActivityCard nestLevel={nestLevel}>
    <CardHeader
      title={actorStr}
      subtitle={dateStr}
    ...
    {activity.isEdited ?
      <p style={styles.inlineNoticeUnderHeader}>
        <em>Last edited {moment(activity.latestEditTime).fromNow()}</em>
      </p> : ''}
...
function DocumentView(props: ActivityViewProps) {
  ...
  const dateStr      = moment(activity.publishTime).fromNow()
  const editDateStr  = activity.isEdited && moment(activity.latestEditTime).fromNow()
  ...
  return <div>
    ...
    {activity.isEdited ?
      <p>
        <em>Last edited {editDateStr} by {editorStr}</em>
      </p> :
      <p>
        <em>Posted {dateStr} by {actorStr}</em>
      </p>}
