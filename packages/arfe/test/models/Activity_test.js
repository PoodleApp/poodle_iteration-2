...
test('produces an activity from a message with HTML content', async t => {
  ...
  t.is(m.count(acts), 1, 'there should be one activity')

  const act = m.first(acts)
  t.is(act.id, msg.uri)
  t.true(m.count(act.objectLinks) > 0, 'activity should have links to content')

  const uri = m.first(act.objectLinks).href
  const parsed = parseMidUri(uri)
  t.truthy(parsed, 'content URI is a `mid:` URI')
  if (!parsed) {
    throw new Error('Expected object to link to a `mid:` resource')
  }

  t.is(parsed.scheme, 'mid:', 'content URI is fully qualified')
  t.is(
    parsed.messageId,
    msg.id,
    'content link points to a part in the same message'
  )
})
...
test('normalizes `actor`', async t => {
  ...
  const acts = await getActivities(fetchPartContent, msg)
  const act = m.first(acts)
  t.is(act.activity.actor.first.id, 'mailto:jesse@sitr.us')
})
...
test('normalizes activity by adding `id`', async t => {
  ...
  const activity = await asutil.exportActivity(AS.create().get())
  const [msg, fetchPartContent] = MB.newMessage({ activity, messageId })

  const acts = await getActivities(fetchPartContent, msg)
  t.is(m.count(acts), 1, 'message contains one activity')
...
test('expands relative `cid:` URIs in activities', async t => {
  ...
  const activity = await asutil.exportActivity(
    AS.create()
      .object(
        AS.note()
          .url([AS.link().mediaType('text/html').href(`cid:${partId}`).get()])
          .get()
      )
      .get()
  )
  ...
  t.is(m.count(links), 1, 'activity contains a link to html content')

  const link = m.first(links)
  t.is(
    link.href,
    `mid:${encodeURIComponent(messageId)}/${encodeURIComponent(partId)}`,
    'href is fully-qualified after normalization'
  )
})
