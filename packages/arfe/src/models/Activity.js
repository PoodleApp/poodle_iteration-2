...
export default class Activity {
  ...
  constructor(activity: AS.models.Activity, message?: Message, part?: MessagePart) {
    const normalized = message ? syncMetadata(activity, message, part) : activity
  ...
  get objectUri(): ?URI {
    const uris = m.map(l => l.href, this.objectLinks)
    return m.first(uris)
  }
...
export async function importActivity(msg: Message, stream: Readable, part?: MessagePart): Promise<Activity> {
...
function syncMetadata(act: AS.models.Activity, msg: Message, part?: MessagePart): AS.models.Activity {
...
function mergeAddresses(xs: ?Address[], ys: ?ValueIterator<AS.models.Base>): AS.models.Object[] {
...
function addressMismatch(addrs: ?Address[], actor: ?ValueIterator<AS.models.Base>): boolean {
  if (!addrs || !actor) { return false }
...
export function participants(activities: Seqable<Activity>): { to:   Seq<Address>
                                                             , from: Seq<Address>
                                                             , cc:   Seq<Address>
                                                             } {
  const { to, from, cc } = m.reduce((ls, activity) => {
    const message = activity.message
    if (!message) { return ls }  // TODO: Fall back to reading participants from activity?
    return {
