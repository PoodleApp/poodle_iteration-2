...
export default class Conversation {
  ...
  get participants(): { to:   Seq<Address>
                      , from: Seq<Address>
                      , cc:   Seq<Address>
                      } {
...
export async function messagesToConversation(
  ...
  const activitiesByMessage = await Promise.all(m.intoArray(
    m.map(Act.getActivities.bind(null, fetchPartContent), messages)
  ))
...
function reduceHelper<R>(f: (r: R, a: DerivedActivity) => R, init: R, acts: Seqable<DerivedActivity>): R {
  if (m.isEmpty(acts)) { return init }
