...
function insertMessage(thread: Thread, message: Activity): Thread {
  const msgs = uniqBy(
    a => a.id,
    m.concat(getActivities(thread), [message])
  )
  const context = msgs
...
function assembleTree(message: Activity, messages: Seqable<Activity>): [Activity, Thread] {
  const context = messages
  const descendents = m.filter(msg => msg !== message && ancestorOf(context, msg, message), messages)
...
export function getActivities(thread: Thread): Seq<Activity> {
  const allActs = m.mapcat(([act, subthread]) => m.cons(act, getActivities(subthread)), thread)
...
function ancestorOf(context: Seqable<Activity>, x: Activity, y: Activity): ?boolean {
...
function ancestors(
  ...
  if (!msg) {
    throw new Error('Cannot determine ancestors of activity with no message context')
  }
...
export function foldThread<T>(reducer: (accum: T, msg: Activity) => T, init: T, thread: Thread): T {
