...
export function query (
  ...
  return basic.requireCapability(capabilities.googleExtensions)
...
export function queryConversations (opts: {
  ...
  return openAllMail().flatMap(() => 
    query(opts.query).flatMap(threadIds => {
      ...
      return Task.par(threadsToFetch.map(threadId =>
        downloadThread(threadId).modifyObservable(obs => obs
          .ignoreValues()
          .beforeEnd(() => threadId)
        )
      ))
...
function getThreadIds (uids: imap.UID[]): Task<ThreadId> {
  ...
  const threadIds = basic.fetch(uids, { bodies: 'HEADER.FIELDS (Message-ID)' })
...
function downloadThread (threadId: ThreadId): Task<URI> {
  return basic.search([['X-GM-THRID', threadId]])
